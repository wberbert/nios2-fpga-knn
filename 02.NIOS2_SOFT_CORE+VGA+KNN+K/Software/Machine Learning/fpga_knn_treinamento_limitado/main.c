...
int main(int argc, char *argv[])
{
    ...
    printf ("02.NIOS2_SOFT_CORE+VGA+KNN+K\n");
    ...
    mint_k_hardware                             = atoi(argv[3]);    //K do software.
    ...
    if (argc >= 5) {
        ...
        mbln_substituicaoparcialdadostreinamento    = atoi(argv[4]);    //Substituicao parcial dos dados de treinamento.

    }

    if (argc >= 6) {

        mbln_excluirregistrosemtreinamento          = atoi(argv[5]);    //Exclui o registro que fez parte do treinamento nas estatisticas.
    ...
    for (int int_registro=0; int_registro<150; int_registro++) {
        ...
        printf ("%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\n", pp_int_dados_teste[int_registro][0], pp_int_dados_teste[int_registro][1], pp_int_dados_teste[int_registro][2], pp_int_dados_teste[int_registro][3], pp_int_dados_teste[int_registro][4], pp_int_dados_teste[int_registro][5], pp_int_dados_teste[int_registro][6], pp_int_dados_teste[int_registro][7], pp_int_dados_teste[int_registro][8]);
    }
...
// ---------------------------------------------------------------------------
    ...
    for (int l=0; l < int_total_linhas_arquivo; l++) {
        ...
        //Tem que contar e nao mais ordenar
        f_ordenar_execucao_parcial(p_reg_distancia, int_total_execucoes_parciais);                          //Ordena lista gerada pelo hardware.
        f_contar_classes(&mint_classe, p_reg_distancia, int_total_execucoes_parciais);                      //Conta as classes.
        ...
        for (int g=0; g < int_total_execucoes_parciais; g++) {
            ...
            printf ("Classe prevista parcial %d\t Classe real %d\t Tempo %5.7f Segundos", p_reg_distancia[g].po_knn_classe_prevista_classe, p_reg_distancia[g].int_classe_real, (float) p_reg_distancia[g].dbl_tempo_ms);
        ...
        p_reg_predicao[l].int_classe_real               = pp_int_dados_teste[l][TOTAL_HARDWARE_ATRIBUTOS - 1];
        ...
        p_reg_predicao[l].int_registrotreinamento       = pp_int_dados_teste[l][TOTAL_HARDWARE_ATRIBUTOS]; // O registro foi utilizado como dados de treinamento. Se sim o certo e exclui-lo das estatisticas.
        #endif // FPGA
        ...
        for (int int_registro=0; int_registro <int_total_linhas_arquivo; int_registro++) {
            ...
            pp_int_dados_teste[int_registro][TOTAL_HARDWARE_ATRIBUTOS] = 0;
    ...
    for (int a=0; a < int_total_registros_selecionados; a++) {
        ...
        switch (enu_algoritmo) {
            case RANDOMICO: //Algoritmo randomico
                ...
                pp_int_dados_teste[int_registro_randomico][TOTAL_HARDWARE_ATRIBUTOS] = 1;
                break;
            ...
            case SEQUENCIALBALANCEADO:
                ...
                for (int b=0; b < int_total_registros_treinamento; b++) {
                    ...
                    if ((pp_int_dados_teste[b][TOTAL_HARDWARE_ATRIBUTOS -1] > int_classe) && (pp_int_dados_teste[b][TOTAL_HARDWARE_ATRIBUTOS] !=1)){

                        pp_int_dados_treinamento[a] = pp_int_dados_teste[b];
                        pp_int_dados_teste[b][TOTAL_HARDWARE_ATRIBUTOS] = 1;

                        int_classe = pp_int_dados_teste[b][TOTAL_HARDWARE_ATRIBUTOS - 1];
                        if ((int_classe % 3) == 0 ) {
                            int_classe = 0;
                        }
                        break;
            ...
            case RANDOMICOEXCLUSIVO: //algoritmo randomico exclusivo
                ...
                } while (pp_int_dados_teste[int_registro_randomico][TOTAL_HARDWARE_ATRIBUTOS] == 1);
...
//Ordena pelo modo bubble sort, pelo cdigo da classe e o pela distncia.
void f_ordenar_execucao_parcial(distancia_t * p_reg, int int_t) {
    ...
    for (int c=0; c < int_t; c++) {
        ...
        for (int d=0; d < (int_t -1); d++) {
            ...
            if (reg_distancia_temp.po_knn_classe_prevista_classe > p_reg[d+1].po_knn_classe_prevista_classe) {
...
void f_contar_classes(int *pint_classe, distancia_t *p_reg, int int_total_execucoes_parciais) {
    int         int_classe = -1;
    int         int_total_classe = 0;
    int         int_maior_total_classe = 0;
    int         int_classe_maior = -1;


    for (int l=0; l < int_total_execucoes_parciais; l++) {

        if (p_reg[l].po_knn_classe_prevista_classe == int_classe) {

            int_total_classe ++;
            if (int_total_classe > int_maior_total_classe) {

                int_maior_total_classe = int_total_classe;
                int_classe_maior = p_reg[l].po_knn_classe_prevista_classe;
            }

        } else {

            int_classe = p_reg[l].po_knn_classe_prevista_classe;
            int_total_classe = 1;
            if (int_classe_maior == -1) {
                int_classe_maior = int_classe;
                int_maior_total_classe = 1;
            }
